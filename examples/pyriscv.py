...
def step(pc, regs, mem):
    ...
    if opcode == "1100110":  # R-type math operations
        ...
        if [funct3, funct7] == [0x0, 0x00]:  # ADD
            regs[rd] = (regs[rs1] + regs[rs2]) % 2**32
        elif [funct3, funct7] == [0x0, 0x20]:  # SUB
            regs[rd] = (regs[rs1] - regs[rs2]) % 2**32
        elif [funct3, funct7] == [0x1, 0x00]:  # SLL
            regs[rd] = (regs[rs1] << (regs[rs2]%32)) % 2**32
            ...
            regs[rd] = (((regs[rs1]+2**31) % 2**32 - 2**31) >> (regs[rs2]%32)) % 2**32
        ...
        if [funct3] == [0x0]:  # ADDI
            regs[rd] = (regs[rs1] + imm) % 2**32
        elif [funct3, funct7] == [0x1, 0x00]:  # SLLI
            regs[rd] = (regs[rs1] << (imm%32)) % 2**32
            ...
            regs[rd] = (((regs[rs1]+2**31) % 2**32 - 2**31) >> (imm%32)) % 2**32
        elif [funct3] == [0x6]:  # OR
            regs[rd] = (regs[rs1] | imm) % 2**32
        elif [funct3] == [0x7]:  # AND
            regs[rd] = (regs[rs1] & imm) % 2**32
        ...
        i = (regs[rs1] + imm) % 2**32
        if [funct3] == [0x0]:  # LB
            regs[rd] = struct.unpack("<b", mem[i:i+1])[0] % 2**32
        elif [funct3] == [0x1]:  # LH
            regs[rd] = struct.unpack("<h", mem[i:i+2])[0] % 2**32
        elif [funct3] == [0x2]:  # LW
            regs[rd] = struct.unpack("<i", mem[i:i+4])[0] % 2**32
        elif [funct3] == [0x4]:  # LBU
        ...
        if [funct3] == [0x0]:  # BEQ
            if regs[rs1] == regs[rs2]: pc[0] = (pc[0] + imm) % 2**32; return
        elif [funct3] == [0x1]:  # BNE
            if regs[rs1] != regs[rs2]: pc[0] = (pc[0] + imm) % 2**32; return
        elif [funct3] == [0x4]:  # BLT
            if regs[rs1]^(1<<31) < regs[rs2]^(1<<31): pc[0] = (pc[0] + imm) % 2**32; return
        elif [funct3] == [0x5]:  # BGE
            if regs[rs1]^(1<<31) >= regs[rs2]^(1<<31): pc[0] = (pc[0] + imm) % 2**32; return
        ...
        regs[rd] = (pc[0] + 4) % 2**32; pc[0] = (pc[0] + imm) % 2**32; return

    elif opcode == "1110011":  # I-type JALR
        rd = btoi(instr[7:12])
        funct3 = btoi(instr[12:15])
        rs1 = btoi(instr[15:20])
        imm = btoi(instr[20:32], signed=True)
        if funct3 == 0:
            regs[rd] = (pc[0] + 4) % 2**32; pc[0] = (regs[rs1] + imm) % 2**32; return
        else:
            assert False, [opcode, funct3]
    ...
    elif opcode == "1100111":  # I-type SYSTEM
        rd = btoi(instr[7:12])
        funct3 = btoi(instr[12:15])
        rs1 = btoi(instr[15:20])
        imm = btoi(instr[20:32], signed=True)
        if [rd, rs1, funct3] == [0, 0, 0x0]:
            return "system", imm
        else:
